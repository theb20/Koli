        ...
        <div className="space-y-6">
          ...
          {!order.isPaid ? (
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl p-4">
              ...
              <div>
                <p className="text-sm font-semibold">En attente de paiement</p>
                <p className="text-xs mt-1 leading-relaxed">
                  Le client, les articles et l'adresse de livraison ne sont communiqués qu'une fois le paiement confirmé —
                  {' '}{order.itemsCount} article{order.itemsCount > 1 ? 's' : ''} pour {fmtFcfa(order.totalAmount)} au total.
                </p>
              </div>
            </div>
          ) : (
          ...
          {order.isPaid && order.customer && order.shippingAddress && (
            <>
              <section>
