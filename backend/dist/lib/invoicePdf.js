...
function drawBrandBar(doc, x, y, width) {
...
function buildInvoicePdf(order, settings) {
    ...
    if (order.promoDiscount > 0)
        totalsRow(`Promo${order.promoCode ? ` (${order.promoCode})` : ''}`, `-${fmt(order.promoDiscount)}`, { color: '#188038' });
    if (order.taxAmount > 0)
        totalsRow(`TVA (${order.taxRate.toFixed(0)}%)`, fmt(order.taxAmount));
    y += 4;
