...
export async function sendOrderStatusEmail(to: string, prenom: string, orderNumber: string, status: string) {
  const statusMsg: Record<string, { title: string; msg: string; emoji: string }> = {
...
export async function sendContactReply(to: string, prenom: string, sujet: string) {
  await transporter.sendMail({
    from: FROM,
    to,
