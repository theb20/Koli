...
export async function sendWelcomeEmail(to: string, prenom: string): Promise<void> {
  ...
  await send(
    to,
    `Bienvenue chez DropShip, ${prenom} 🎉`,
    baseLayout(`
      ${subheading('Compte activé')}
      ${heading(`Bienvenue, ${prenom} !`)}
      ${paragraph('Votre compte DropShip est prêt. Vous rejoignez <strong style="color:#111827">+12&nbsp;000</strong> clients satisfaits à travers le France.')}
      ...
      ${highlightBox(`
        <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="border-collapse:collapse">
          <tbody>
            ...
            ${iconRow('🎁', '<strong style="color:#111827">Offres exclusives</strong> · Réservées aux membres Koli')}
          </tbody>
        </table>
      `)}
      ...
      `, `Bienvenue chez DropShip, ${prenom} — votre compte est actif.`),
  )
}
