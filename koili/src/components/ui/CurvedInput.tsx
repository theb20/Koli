...
interface CurvedInputProps {
  ...
  className?: string;
  style?: CSSProperties;
  /** Bloque la saisie et l'envoi (ex: requête en cours) — l'input natif
   * caché devient un vrai <input disabled>, donc non focusable. */
  disabled?: boolean;
}
...
const CurvedInput = ({
  ...
  if (geom && layout) {
    ...
    content = (
      <svg
        ref={svgRef}
        className={`block w-full h-auto overflow-visible [&_text]:font-[inherit] select-none [-webkit-tap-highlight-color:transparent] transition-opacity duration-150 ${disabled ? 'opacity-60 cursor-not-allowed' : 'cursor-text'}`}
        width={geom.W}
